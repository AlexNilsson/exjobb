...
class VAE:
  ...
  def buildDecoder(self):
    ...
    x = Conv2D(512, 3, activation='tanh', padding='same')(x)
    x = BatchNormalization(momentum=0.7)(x)

    x = UpSampling2D()(x)
    x = Conv2D(256, 3, activation='tanh', padding='same')(x)
    x = BatchNormalization(momentum=0.7)(x)

    x = UpSampling2D()(x)
    x = Conv2D(128, 3, activation='tanh', padding='same')(x)
    x = BatchNormalization(momentum=0.7)(x)

    x = UpSampling2D()(x)
    x = Conv2D(64, 3, activation='tanh', padding='same')(x)
    x = BatchNormalization(momentum=0.7)(x)
