...
#----------------------------------------------
""" user preferences """
pixels_amount = 80 #must be dividable by 8
batchsize = 500 #the trainingset must be dividable with batches_size
n_epoch = 10000
hidden_1_size =500 
...
SAVE_IMAGE_ON_EPOCH = 50
SAVE_WEIGHTS_ON_EPOCH = 2000

LOAD_WEIGHTS_TO_MODEL=False
PREVIOUSLY_RUN_EPOCH = 0
...
train_images_path = os.path.join(os.path.dirname(__file__), '../data/windows/original_images')
train_images_resized_path = os.path.join(os.path.dirname(__file__), '../data/windows/resized_images')
...
""" Convolution layers """
conv_1 = Conv2D(16, (3, 3), padding='same')
pooling_1 = MaxPooling2D((2, 2), padding='same')
conv_2 = Conv2D(8, (3, 3), padding='same')
pooling_2 = MaxPooling2D((2, 2), padding='same')
conv_3 = Conv2D(8, (3, 3), padding='same')
pooling_3 = MaxPooling2D((2, 2), padding='same')
lrelu = LeakyReLU()

""" Flat layers """
flat_hidden_1 =Dense(hidden_1_size)
flat_hidden_2 = Dense(hidden_2_size)
...
""" Structure """
inputs_encoder = Input(shape=(pixels_amount, pixels_amount, 1))
x = conv_1(inputs_encoder)
x = LeakyReLU()(x)
x = pooling_1(x)
x = conv_2(x)
x = LeakyReLU()(x)
...
lr1 = LeakyReLU()(h_q)
h_q_2 = flat_hidden_2(lr1)
#lr2 = LeakyReLU()(h_q_2)
z = flat_2(h_q_2)
#mu = flat_2(h_q_2)
#log_sigma = flat_3(h_q_2)
encoder = Model(inputs_encoder, z)
...
""" Layers """
decoder_hidden_2 = Dense(hidden_2_size)
...
decoder_hidden = Dense(hidden_1_size)
d_h = Dense(_x*_x*_z)
d_h_reshaped = Reshape((_x,_x,_z))
decon_1 = Conv2D(8, (3, 3), padding='same')
upsamp_1 = UpSampling2D((2, 2))
decon_2 = Conv2D(8, (3, 3), padding='same')
upsamp_2 = UpSampling2D((2, 2))
decon_3 = Conv2D(16, (3, 3), padding='same')#, padding='same'
...
""" Structure """
inputs_decoder = Input(shape=(z_layer_size,))
x = decoder_hidden_2(inputs_decoder)
x = LeakyReLU()(x)
...
""" x = decon_4(x)
x =upsamp_4(x) """
outputs_decoder = decon_4(x)
decoder = Model(inputs_decoder, outputs_decoder)
#---------------------------------------------------------------------------------------------------------------------------------
#----------------------------------------------

#VAE model, for reconstruction and training
""" Structure """
inputs_ae = Input(shape=(pixels_amount, pixels_amount, 1))
x = encoder(inputs_ae)
outputs_ae = decoder(x)
vae = Model(inputs_ae, outputs_ae)
...
inputs_flat = Flatten()(inputs_encoder)
outputs_flat = Flatten()(outputs_decoder)
...
#layers
disc_1 = Dense(disc_1_size)
disc_2 = Dense(disc_2_size)
disc_3 = Dense(disc_3_size)
disc_4 = Dense(disc_4_size)
disc_5 = Dense(disc_5_size)
...
inputs_disc_enc = Input(shape=(pixels_amount, pixels_amount, 1))
x = encoder(inputs_disc_enc)
output_disc_enc = discriminator(x)
disc_enc = Model(inputs_disc_enc, output_disc_enc)
''' x = disc_1(z)
x = disc_2(x)
x = disc_3(x)
x = disc_4(x)
...
disc_enc = Model(inputs, discriminator_e_output) '''
#---------------------------------------------------------------------------------------------------------------------------------
#----------------------------------------------
...
def load_pretrained_weights():
    vae.load_weights('./VariationalAE/saved_weights/vae_weights_epoch_{}.h5'.format(PREVIOUSLY_RUN_EPOCH), by_name=False)
    disc_enc.load_weights('./VariationalAE/saved_weights/disc_enc_weights_epoch_{}.h5'.format(PREVIOUSLY_RUN_EPOCH), by_name=False)
    encoder.load_weights('./VariationalAE/saved_weights/encoder_weights_epoch_{}.h5'.format(PREVIOUSLY_RUN_EPOCH), by_name=False)
    decoder.load_weights('./VariationalAE/saved_weights/decoder_weights_epoch_{}.h5'.format(PREVIOUSLY_RUN_EPOCH), by_name=False)
    discriminator.load_weights('./VariationalAE/saved_weights/discriminator_weights_epoch_{}.h5'.format(PREVIOUSLY_RUN_EPOCH), by_name=False)

if LOAD_WEIGHTS_TO_MODEL==True:
...
disc_enc.compile(optimizer='adam', loss='binary_crossentropy')
...
def saving_weights(epoch):
    vae.save_weights('./VariationalAE/saved_weights/vae_weights_epoch_{}.h5'.format(epoch + PREVIOUSLY_RUN_EPOCH))
    discriminator.save_weights('./VariationalAE/saved_weights/discriminator_weights_epoch_{}.h5'.format(epoch + PREVIOUSLY_RUN_EPOCH))
    decoder.save_weights('./VariationalAE/saved_weights/decoder_weights_epoch_{}.h5'.format(epoch + PREVIOUSLY_RUN_EPOCH))
    encoder.save_weights('./VariationalAE/saved_weights/encoder_weights_epoch_{}.h5'.format(epoch + PREVIOUSLY_RUN_EPOCH))
    disc_enc.save_weights('./VariationalAE/saved_weights/disc_enc_weights_epoch_{}.h5'.format(epoch + PREVIOUSLY_RUN_EPOCH))
...
def epoch_vis(self, epoch):
    ...
    figure_to_file = figure*255 #reshape to get the right range when saving image to file
    figure_to_file = figure_to_file.astype('uint8')
    name_of_image = './VariationalAE/epoch_plots/{}.jpg'.format(epoch + PREVIOUSLY_RUN_EPOCH)
    c.imwrite(name_of_image, figure_to_file)
...
for epochnumber in range(n_epoch):
    ...
    for i in range(int(len(x_train) / batchsize)):
        ...
        disc_enc.train_on_batch(batch, np.ones(batchsize)) #trains the encoder to code z-values to fit in the wanted distribution
    
    print('---------------------------------------------------------------- epoch',epochnumber + PREVIOUSLY_RUN_EPOCH, 'of ' '{}'.format(n_epoch + PREVIOUSLY_RUN_EPOCH))
    print("Reconstruction Loss:", vae.evaluate(x_train, x_train, verbose=1))
    print("Adverserial Loss:", disc_enc.evaluate(x_train, np.ones(len(x_train)), verbose=1))

    if epochnumber % SAVE_IMAGE_ON_EPOCH == 0:
        #imagegrid(decoder, epochnumber)
        epoch_vis(decoder, epochnumber)
    if epochnumber % SAVE_WEIGHTS_ON_EPOCH == 0:
        saving_weights(epochnumber)
#---------------------------------------------------------------------------------------------------------------------------------
#----------------------------------------------
...
disc_enc.save_weights('disc_enc_weights.h5')
#---------------------------------------------------------------------------------------------------------------------------------
#----------------------------------------------
# visualisation
