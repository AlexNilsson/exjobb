...
for file in listing:
    ...
    #gray = img.convert('L')
    img.save(train_images_resized_path + '\\' + file ) #+ ".jpg"
...
x_train = np.reshape(x_train, (len(x_train), pixels_amount, pixels_amount, 3))  # adapt this if using `channels_first` image data format
x_test = np.reshape(x_test, (len(x_test), pixels_amount, pixels_amount, 3))  # adapt this if using `channels_first` image data format
print(x_train.shape)
print(x_test.shape)
...
""" Structure """
inputs = Input(shape=(pixels_amount, pixels_amount, 3))
x = conv_1(inputs)
x = pooling_1(x)
x = conv_2(x)
x = pooling_2(x)
...
decon_4 = Conv2D(3, (3, 3), activation='sigmoid', padding='same')
...
""" vae.save('./VariationalAE/vae_model.h5')
vae.save_weights('./VariationalAE/vae_weights.h5')
decoder.save_weights('./VariationalAE/decoder_weights.h5')
encoder.save_weights('./VariationalAE/encoder_weights.h5')
decoder.save('./VariationalAE/decoder_model.h5')
vikter = decoder.get_weights()
print(vikter) """
#---------------------------------------------------------------------------------------------------------------------------------
#----------------------------------------------
# visualisation
...
figure = np.zeros((visualisation_size * n, visualisation_size * n, 3))
...
for i, yi in enumerate(grid_x):
    for j, xi in enumerate(grid_y):
        ...
        visualisation_image = x_decoded[0].reshape(visualisation_size, visualisation_size, 3)
...
plt.savefig(
  "./VariationalAE/progress_images/color_epoch{}-h1_size{}-h2_size{}-dropout{}.jpg".format(n_epoch, hidden_1_size, hidden_2_size, dropout_amount)
  )
plt.show()
