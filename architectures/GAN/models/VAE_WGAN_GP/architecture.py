...
class GAN:
  ...
  def __init__(self):
    ...
    self.vae_decoder = load_model(os.path.join(C.VAE_DECODER,'decoder_model.h5'))
    self.vae_decoder.load(os.path.join(C.VAE_DECODER,'decoder_weights.h5'))
  ...
  def _buildGenerator(self, input_noise):
    ...
    x = self.vae_decoder.predict(x)
    # (64, 64, 3)

    #? Block 1
    x = Conv2DTranspose(256, 4, strides=2)(x)
