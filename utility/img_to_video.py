...
"""
py img_to_video.py -d 'path/to/dir' -o rendered.avi -fps 25
"""
import math
import cv2
import argparse
...
def image_sort_d(x,y):
  x = int(x.split("-")[0])
  y = int(y.split("-")[0])
  return x-y

# Returns a frame sequence following a slow growing exponential
def getFrameSequence(maxFrame, damping = 10, exp=1):
  frames = []
  i = frame_idx = 0
  while frame_idx < maxFrame:
    frames.append(frame_idx)
    i += 1
    frame_idx += math.ceil(i/damping)**exp
  return frames


# Construct the argument parser and parse the arguments
arg_parser = argparse.ArgumentParser()
arg_parser.add_argument("-e", "--extension", required=False, default='jpg', help="Extension name. default is 'jpg'.")
arg_parser.add_argument("-o", "--output", required=False, default='output.avi', help="Output video file.")
...
if sort_type == "numeric":
  if isnum(images[0].split(".")[0]):
    images = sorted(images, key=cmp_to_key(image_sort))

  elif isnum(images[0].split("-")[0]):
    images = sorted(images, key=cmp_to_key(image_sort_d))
...
frames_to_save = getFrameSequence(len(images), damping=200, exp=3)
images_to_save = [images[i] for i in frames_to_save]

for n, image in enumerate(images_to_save):

  if n % 10 == 0: print("Frame " + str(n))
